...
The same logic applies to a real control task. Four DQN policies were trained on LunarLander-v3 (training seeds 0–3) and evaluated across 200 terrains; the returns are bundled.
...
Two further limits are worth stating. The sharpest LunarLander result — the solved-or-not coin flip — is largest precisely because the policy sits near the threshold; a clearly solved or clearly failed policy would not show it, though the best-policy and reliability results do not depend on proximity to the threshold. And the bootstrap confidence treats the evaluated terrains as the population, so it quantifies sampling noise, not a mismatch between the evaluation and deployment terrain distributions.
