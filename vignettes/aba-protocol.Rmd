...
The same logic applies to a real control task. Five policies were trained on LunarLander-v3 and evaluated across 200 terrains; the returns are bundled. Four are DQN policies (training seeds 0–3, near-solved or worse); the fifth (policy seed 99) is a PPO policy trained to a verified solve, with a true mean above 240.
...
Exactly one of the five policies — the PPO policy (seed 99) — meets the criterion, at a settled estimate near 243. Even for that genuinely solved policy a very short evaluation occasionally reads not solved, while the protocol confirms it with full bootstrap confidence; the protocol is thus correct in both directions (false positives on the near-threshold DQN policies, false negatives on the solved one).
