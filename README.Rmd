...
The same logic applies to a real control task. Five policies were trained on LunarLander-v3 and evaluated across 200 terrains; the returns are bundled. Four are DQN policies (training seeds 0–3); the fifth is a PPO policy trained to a **verified solve** — a true mean of about 243 over 200 terrains, above the canonical 200 threshold, with far lower variance than the DQN policies.
...
*Which trained seed is reliable?* At a fixed budget the seeds split, and exactly one of the five — the PPO policy — clears the bar:
