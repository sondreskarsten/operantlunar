...
```{r hero, echo = FALSE, fig.height = 3.0, fig.alt = "Ad hoc verdicts spread across outcomes while the protocol returns a single verdict"}
library(operantlunar)
plot_lunar_convergence(lunar_solved_convergence(policy_seed = 0L))
```
...
In RStudio, open the project and use the **Build** pane (Install, Check, Document). The package is native R; the Python LunarLander environment is optional and only needed for the `lunar_setup()` / `differentiate()` path.

## Quickstart: watch a solved policy land

The package bundles a verified-solved PPO policy (true mean ~243 over 200 terrains) and a runnable example that lets you **watch it fly**. Run:

```{r quickstart, eval = FALSE}
source(system.file("examples", "lunar_quickstart.R", package = "operantlunar"))
```

By default it opens a real-time window and plays several fresh episodes on random terrains — genuine rollouts, not a replay — printing each return. On a headless machine it falls back to writing a GIF of those same runs.

The preview below is not a single cherry-picked clip: it is four actual episodes on different terrains (random seeds 11, 22, 33, 44; returns 236, 214, 260, 285), all landing.
...
The example drives Gymnasium through `reticulate`; the environment and `stable-baselines3` are provisioned on first use, or point `lunar_setup("/path/to/python")` at an interpreter that already has `gymnasium[box2d]`, `stable-baselines3` and `pillow`.
...
The protocol's value shows up as conclusion stability. *Is a policy solved?* A five-episode evaluation is a coin flip near the threshold; the protocol reads a settled estimate against the frozen threshold with a bootstrap confidence.
...
*Which trained seed is reliable?* At a fixed budget the seeds split — the four PPO policies clear the bar, the four DQN policies do not:
...
The bundled Shiny app exposes the operant battery, the ABA-on-Gymnasium instruments, and a **Protocol value-add** tab that stress-tests the is-it-solved and best-policy conclusions on the bundled returns, contrasting the budget-dependent ad hoc verdict with the invariant protocol verdict.

## Finding your way around

143 exported functions, organized on the [pkgdown reference index](https://sondreskarsten.github.io/operantlunar/reference/). The main families:
...
- *Differentiating reward maximization from melioration* — the namesake instrument.
