...
![Ad hoc verdicts spread across outcomes while the protocol returns a
single verdict](reference/figures/README-hero-1.png)
...
is optional and only needed for the
...
— which is agent- and environment-agnostic. Only the **update rule** and
the **environment** change. Three layers build on it.
...
tabulates each instrument against the RL problem it reframes;
...
a standardized criterion-line decision rule
([`criterion_line_verdict()`](https://sondreskarsten.github.io/operantlunar/reference/criterion_line_verdict.md),
after Hagopian et al. 1997), and idiographic replication that never
pools across subjects
